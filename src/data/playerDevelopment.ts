...
export interface Appearance { skin: string; hair: string; hairStyle: 'short' | 'long' | 'crop'; uniform: string; accessory: 'none' | 'glasses' | 'headband' }
...
export function pitchingRating(p: Player) {
 const n=normalizePlayer(p); const arsenal=n.pitches!.filter(x=>x.rating>0);
 const pitch=arsenal.length ? arsenal.reduce((s,x)=>s+x.rating,0)/arsenal.length : 0;
 return (n.stuff+n.movement!+n.control+n.stamina+n.holdRunners!+pitch+Math.max(0,Math.min(100,(n.velocity!-100)*2)))/7;
}
export function battingRating(p: Player) { const n=normalizePlayer(p); return (n.contact+n.power+n.eye+n.gapPower!+n.avoidK!+n.speed+n.stealing!+n.baserunning!+n.fieldingRange!+n.fieldingError!+n.arm!)/11; }
